// ==========================================
// 🚀 NITROUS BOOSTER v3 (CDN SAFE DELAYED)
// ==========================================
(function() {
    'use strict';
    ...
    // FASE 2: Fetch Interceptor diaktifkan SETELAH web selesai load
    function activateNitrousBoost() {
        window.fetch = async function(...args) {
            ...
            // AMAN: Hanya cache milik NathanSecurity, API lain (admin.php) dilewatkan mentah-mentah
...
// ==========================================
// 🛡️ INVISIBLE SECURITY SHIELD v3 (CDN SAFE DELAYED)
// ==========================================
(function() {
    'use strict';
    ...
    // FASE 2: Override Storage (Ditunda agar tidak mengganggu proses login/init web host)
    function activateStorageShield() {
    ...
    // ==========================================
    // FASE 1: BERJALAN INSTANT (Hanya Event Listener Dasar)
    // ==========================================
    ...
    document.addEventListener('copy', function(e) {
        ...
        if (sel.length > 0) {
            if (/@gmail\.com|@yahoo\.com|@hotmail\.com|@outlook\.com|user[_-]?token|api[_-]?key|access[_-]?token|password|credit[_-]?card|ssn|passport|secret|private[_-]?key|bearer\s+|authorization|cookie|session[_-]?id/i.test(sel)) { 
    ...
    // ==========================================
    // FASE 2: BERJALAN SETELAH WEB 100% SELESAI LOAD + DELAY AMAN
    // ==========================================
    window.addEventListener('load', function() {
        
        // DELAY 2 DETIK: Menunggu Database admin.php selesai diproses & dirender
        setTimeout(function() {

            // 1. Aktifkan Nitrous Boost & Cache
            if (typeof window.activateNitrousBoost === 'function') window.activateNitrousBoost();
            
            // 2. Aktifkan Storage Encryption
            activateStorageShield();

            // 3. Aktifkan Cleaner (Hapus komentar HTML saja, AMAN untuk layout)
            if (typeof window.activateCleaner === 'function') window.activateCleaner();

            // 4. Anti XSS (Diperbaiki: Membiarkan script dengan URL http/https agar tidak crash jQuery/Axios)
            ...
            new MutationObserver(mutations => {
                mutations.forEach(m => m.addedNodes.forEach(node => {
                    if (node.nodeName === 'SCRIPT') {
                        if (node.src && /^https?:\/\//.test(node.src)) return; // AMAN: Biarkan script dari URL manapun (Bootstrap, Axios, dll)
            ...
            // 5. Pasang Popup Klik Kanan Mewah (Seperti Versi Asli)
        ...
        }, 2000); // <--- DELAY 2 DETIK MENUNGGU DB WEB PELANGGAN SELESAI
    });
